...
import EnterAddressForm from "@/components/profile/EnterAddressForm";
import ProfileDetails from "@/components/profile/ProfileDetails";
import VerifyMobile from "@/components/profile/VerifyMobile";
...
export default function MobileVerificationScreen() {
	...
	const isFormValid = () => {
		return Object.entries(addressDetails).every(([key, value]) =>
			key === "phone" || key === "landmark" ? true : (value as string).trim() !== ""
		);
	};
		...
		<SafeAreaView style={{ flex: 1 }}>
			{!showAddressForm && !address && (
				<VerifyMobile
					...
					otpDigits={otpDigits}
					otpRefs={otpRefs}
					onChangeOtpBox={handleBoxChange}
					onKeyPressOtpBox={handleBoxKeyPress}
					otpSent={otpSent}
					error={error}
					infoHint={infoHint}
